    ...
    <div class="ui segment" v-bind:class="{ inverted: dark }">
        <div class="ui centered two column grid">
            <div class="ui centered column">
                ...
                @if (Auth::user()->can('update', $team)) <a href="{{ route('teamadmin', ['teamid' => $team->id]) }}" class="ui primary labeled icon button"><i class="edit icon"></i> {{ __('profile.edit') }}</a> @endif
                <br><br>
                ...
                <div class="ui list">
                    ...
                    <div class="item">
                        ...
                        <div class="right floated content">
                            @if (Auth::user()->can('kickMember', $team) && Auth::id() != $user->id) <a href="{{ route('teamadminkick', ['teamid' => $team->id, 'userid' => $user->id]) }}" class="ui orange tiny button">{{ __('profile.kick') }}</a> @endif
                            @if (Auth::id() == $user->id && Auth::user()->cannot('kickMember', $team)) <a href="{{ route('teamadminkick', ['teamid' => $team->id, 'userid' => $user->id]) }}" class="ui orange tiny button">{{ __('team.member.quit') }}</a> @endif
                        </div>
                    </div>
                    @endforeach
                </div>
